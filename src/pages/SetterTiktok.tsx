...
import { Search, ListFilter as Filter, Eye, Music, TrendingUp } from 'lucide-react';
...
export function SetterTiktok() {
    ...
    async function fetchContacts() {
        try {
            ...
            console.error('Error fetching setter tiktok contacts:', error);
        } finally {
            setLoading(false);
        }
    }
    ...
    async function fetchCommissions() {
        try {
            ...
            console.error('Error fetching setter tiktok commissions:', error);
        }
    }
        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center text-white">
                <div>
                    <h1 className="text-3xl font-bold flex items-center gap-2">
                        ...
                        Setter TikTok (Source s-t)
                    </h1>
                    <p className="text-slate-400 text-sm mt-1">Consultation des contacts provenant du setter TikTok.</p>
                </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-slate-800 border border-slate-700 p-6 rounded-2xl">
                    <p className="text-slate-400 text-sm font-medium mb-1">Total Contacts s-t</p>
            ...
            <div className="flex gap-4">
                <div className="relative flex-1">
                    ...
                    <input
                        type="text"
                        placeholder="Rechercher parmi les contacts s-t..."
                        value={searchTerm}
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm text-slate-400 min-w-[600px]">
                        ...
                        <tbody className="divide-y divide-slate-700">
                            {loading ? (
                                <tr>
                                    <td colSpan={5} className="px-6 py-8 text-center text-slate-500 italic">Chargement des données setter TikTok...</td>
                                </tr>
                            ) : filteredContacts.length === 0 ? (
                                <tr>
                                    <td colSpan={5} className="px-6 py-8 text-center text-slate-500">Aucun contact "s-t" trouvé</td>
                                </tr>
                            ) : (
